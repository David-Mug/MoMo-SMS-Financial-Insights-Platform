...
#Load the xml file and buiild the tree structure
tree = ET.parse("modified_sms_V2.xml")
#access the root element
root = tree.getroot()
#create an empty list to store the transaction attributes we need.
transactions = []
#loop over the "sms" element representing transactions
for identity, sms in enumerate(root.findall("sms"), start=1):
